...
local function tick_poll_station(map_data, mod_settings)
	...
	station.inactivity_time_load = nil
	station.inactivity_time_unload = nil
		...
		for k, v in pairs(comb1_signals) do
			...
			if item_name then
				if item_type == "virtual" then
					...
					elseif item_name == INACTIVITY_TIME_LOAD then
						station.inactivity_time_load = item_count > 0 and item_count*60 or nil -- convert to ticks
					elseif item_name == INACTIVITY_TIME_UNLOAD then
						station.inactivity_time_unload = item_count > 0 and item_count*60 or nil -- convert to ticks
					elseif is_each then
